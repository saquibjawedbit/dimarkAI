...
export class GeminiService {
  ...
  async rephraseText(request: RephraseTextRequest): Promise<GeminiResponse> {
    try {
      const response = await apiService.post(`${this.basePath}/rephrase`, request);
  ...
  async generateAdText(request: GenerateTextRequest): Promise<GeminiResponse> {
    try {
      const response = await apiService.post(`${this.basePath}/generate`, request);
      return response as GeminiResponse;
    } catch (error: any) {
  ...
  async getOptions(): Promise<{ success: boolean; data?: GeminiOptions; error?: string }> {
    try {
      const response = await apiService.get(`${this.basePath}/options`);
      return response as unknown as { success: boolean; data?: GeminiOptions; error?: string };
    } catch (error: any) {
